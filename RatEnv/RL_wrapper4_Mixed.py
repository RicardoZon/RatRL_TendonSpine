...
class RatRL(gym.Env):
    def __init__(self, xml_file, fre_cyc=0.67, Render=False, Pars=None):
        super(RatRL, self).__init__()
        # Wrapper
        high_action = np.array([1.0] * 5).astype(np.float32)

        high = np.array([np.inf] * (15+len(high_action))).astype(np.float32)
        ...
        self.frame_skip = 15  # 02s 10
        if Pars:
            self.frame_skip = Pars['frame_skip']

        self.model.opt.timestep = 0.002  # Default
    ...
    def step(self, action, LegCal=False):
        ...
        action = np.array([0., action[0], 0., action[1], 0., action[2], 0., action[3], action[4]])
        action = np.insert(action, 8, [0] * 3)
        ranges = [1.57] * 8 + [0., 0., 0., 0.8]
        delta = action * ranges
        ...
        if sum_contact == 0:
            ...
            self.nair = 0
        # if self.nair == 120:  # 10 For 65 20 For 66  120 For 360  # Problem  terminal too early
        #     # Trapped
